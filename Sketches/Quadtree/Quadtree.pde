...
// another example
final static String assets_folder = "assets/";
final static String sub_folder = "typo/";
final static String file_prefix = "t_";
final static String pattern_prefix = assets_folder+sub_folder+file_prefix;

final static String file_ext = ".jpg";
final static int pattern_init = 0; // starting number
 final static int pattern_length = 20; // how many images from the set
...
void setup() {
  ...
  img = loadImage(foldername+source_folder+filename+fileext);
...
void draw() {
  ...
  exit();
  //if(done) image(buffer, 0, 0, width, height);
...
void saveOutput() {
    String fn = foldername + "export/" + filename +"/"+frame+"_" +"_t_"+THR+"__m_"+MINR+"_"+ filename+fileext;
    buffer.save(fn);
